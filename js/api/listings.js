...
export async function getAllListings() {
    try {
        const res = await fetch(`${BASE}?_seller=true&_bids=true&limit=100&sort=created&sortOrder=desc`, { headers });
...
export async function createListing(data, token) {
    try {
        ...
        if (!res.ok) {
            ...
            throw new Error(err.errors?.[0]?.message || `Failed to create listing (${res.status})`);
        }
