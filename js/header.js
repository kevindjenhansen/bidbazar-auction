    ...
    header.innerHTML = `
      <div class="flex items-center gap-3 px-4 py-2 rounded-[15px] border border-[#003BFF]" style="background:rgba(70,166,255,0.4)">
        ...
        <div>
          ...
          <div class="font-bold text-[#2482ED]" id="headerCredits">${userProfile.credits ?? 0}</div>
        </div>
      </div>
      <a href="/html/profile/profile.html" class="flex items-center gap-2 hover:opacity-80">
        <img src="${userProfile.avatar?.url || "/assets/logo/bidbazar-logo.png"}" class="w-10 h-10 rounded-full border object-cover" />
        <span class="label-text font-semibold hidden sm:block"> ${userProfile.name} </span>
      </a>
